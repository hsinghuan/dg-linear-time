...
from src.models.modules.time import CosineTimeEncoder, LinearTimeEncoder
...
class MemoryModel(torch.nn.Module):
    ...
    def __init__(
        ...
        num_heads: int = 2,
        dropout: float = 0.1,
        src_node_mean_time_shift: float = 0.0,  # used in JODIE but not in TGN and DyRep
        src_node_std_time_shift: float = 1.0,  # used in JODIE but not in TGN and DyRep
        dst_node_mean_time_shift_dst: float = 0.0,  # used in JODIE but not in TGN and DyRep
        dst_node_std_time_shift: float = 1.0,  # used in JODIE but not in TGN and DyRep
        time_encoding_method: str = "sinusoidal",
        avg_time_diff: float = None,  # used for TGN to scale time differences
        std_time_diff: float = None,  # used for TGN to scale time differences
        device: str = "cpu",
    ):
        """
        ...
        if time_encoding_method == "sinusoidal":
            ...
            raise ValueError(
                f"Not implemented error for time_encoding_method {time_encoding_method}!"
            )
    ...
    def get_updated_memories(self, node_ids: np.ndarray, node_raw_messages: dict):
        """Get the updated memories based on node_ids and node_raw_messages (just for computation),
        but not update the memories :param node_ids: ndarray, shape (num_nodes, ) :param
        node_raw_messages: dict, dictionary of list, {node_id: list of tuples}, each tuple is
        (message, time) with type (Tensor shape (message_dim, ), a scalar) :return:"""
    ...
    def update_memories(self, node_ids: np.ndarray, node_raw_messages: dict):
        """Update memories for nodes in node_ids :param node_ids: ndarray, shape (num_nodes, )
        :param node_raw_messages: dict, dictionary of list, {node_id: list of tuples}, each tuple
        is (message, time) with type (Tensor shape (message_dim, ), a scalar) :return:"""
        # aggregate messages for the same nodes
        # unique_node_ids, ndarray, shape (num_unique_node_ids, ), array of unique node ids
        # unique_node_messages, Tensor, shape (num_unique_node_ids, message_dim), aggregated messages for unique nodes
        # unique_node_timestamps, ndarray, shape (num_unique_node_ids, ), array of timestamps for unique nodes
    ...
    def compute_new_node_raw_messages(
        ...
        new_src_node_raw_messages = torch.cat(
            [src_node_memories, dst_node_memories, src_node_delta_time_features, edge_features],
            dim=1,
        )
...
class MessageAggregator(nn.Module):
    def __init__(self):
        ...
        Given a batch of node ids and corresponding messages, aggregate messages with the same node
        id.
        """
        super().__init__()
    ...
    def aggregate_messages(self, node_ids: np.ndarray, node_raw_messages: dict):
        ...
        messages with the same node id (only keep the last message for each node) :param node_ids:

        ndarray, shape (batch_size, )
        :param node_raw_messages: dict, dictionary of list, {node_id: list of tuples}, each tuple
            is (message, time) with type (Tensor shape (message_dim, ), a scalar)
        :return:
        """
...
class MemoryBank(nn.Module):
    ...
    def __init_memory_bank__(self):
        """Initialize all the memories and node_last_updated_times to zero vectors, reset the
        node_raw_messages, which should be called at the start of each epoch :return:"""
        self.node_memories.data.zero_()
        self.node_last_updated_times.data.zero_()
        self.node_raw_messages = defaultdict(list)

    def get_memories(self, node_ids: np.ndarray):
        """Get memories for nodes in node_ids :param node_ids: ndarray, shape (batch_size, )
        :return:"""
    ...
    def set_memories(self, node_ids: np.ndarray, updated_node_memories: torch.Tensor):
        """Set memories for nodes in node_ids to updated_node_memories :param node_ids: ndarray,
        shape (batch_size, ) :param updated_node_memories: Tensor, shape (num_unique_node_ids,
        memory_dim) :return:"""
        self.node_memories[torch.from_numpy(node_ids)] = updated_node_memories

    def backup_memory_bank(self):
        """Backup the memory bank, get the copy of current memories, node_last_updated_times and
        node_raw_messages :return:"""
    ...
    def reload_memory_bank(self, backup_memory_bank: tuple):
        """Reload the memory bank based on backup_memory_bank :param backup_memory_bank: tuple
        (node_memories, node_last_updated_times, node_raw_messages) :return:"""
    ...
    def detach_memory_bank(self):
        """Detach the gradients of node memories and node raw messages :return:"""
    ...
    def store_node_raw_messages(self, node_ids: np.ndarray, new_node_raw_messages: dict):
        """Store raw messages for nodes in node_ids :param node_ids: ndarray, shape (batch_size, )
        :param new_node_raw_messages: dict, dictionary of list, {node_id: list of tuples}, each
        tuple is (message, time) with type (Tensor shape (message_dim, ), a scalar) :return:"""
    ...
    def clear_node_raw_messages(self, node_ids: np.ndarray):
        """Clear raw messages for nodes in node_ids :param node_ids: ndarray, shape (batch_size, )
        :return:"""
        for node_id in node_ids:
            self.node_raw_messages[node_id] = []

    def get_node_last_updated_times(self, unique_node_ids: np.ndarray):
        """Get last updated times for nodes in unique_node_ids :param unique_node_ids: ndarray,
        (num_unique_node_ids, ) :return:"""
...
class MemoryUpdater(nn.Module):
    ...
    def update_memories(
        ...
        """Update memories for nodes in unique_node_ids :param unique_node_ids: ndarray, shape
        (num_unique_node_ids, ), array of unique node ids :param unique_node_messages: Tensor,
        shape (num_unique_node_ids, message_dim), aggregated messages for unique nodes :param
        unique_node_timestamps: ndarray, shape (num_unique_node_ids, ), timestamps for unique nodes
        :return:"""
    ...
    def get_updated_memories(
        ...
        """Get updated memories based on unique_node_ids, unique_node_messages and
        unique_node_timestamps (just for computation), but not update the memories :param
        unique_node_ids: ndarray, shape (num_unique_node_ids, ), array of unique node ids :param
        unique_node_messages: Tensor, shape (num_unique_node_ids, message_dim), aggregated messages
        for unique nodes :param unique_node_timestamps: ndarray, shape (num_unique_node_ids, ),
        timestamps for unique nodes :return:"""
...
class TimeProjectionEmbedding(nn.Module):
    ...
    def compute_node_temporal_embeddings(
        self, node_memories: torch.Tensor, node_ids: np.ndarray, node_time_intervals: torch.Tensor
    ):
        """Compute node temporal embeddings using the embedding projection operation in JODIE
        :param node_memories: Tensor, shape (num_nodes, memory_dim) :param node_ids: ndarray, shape
        (batch_size, ) :param node_time_intervals: Tensor, shape (batch_size, ) :return:"""
        # Tensor, shape (batch_size, memory_dim)
...
class GraphAttentionEmbedding(nn.Module):
    def __init__(
        ...
        neighbor_sampler: NeighborSampler,
        time_encoder: Union[CosineTimeEncoder, LinearTimeEncoder],
        node_feat_dim: int,
        edge_feat_dim: int,
        time_feat_dim: int,
        output_dim: int = 172,
        ...
        if self.node_feat_dim < self.output_dim:
            self.projection_layer = nn.Linear(
                in_features=self.node_feat_dim, out_features=self.output_dim, bias=True
            )
        else:
    ...
    def compute_node_temporal_embeddings(
        ...
        """Given memory, node ids node_ids, and the corresponding time node_interact_times, return
        the temporal embeddings after convolution at the current_layer_num :param node_memories:
        ...
        if self.projection_layer is not None:
            node_features = node_memories[torch.from_numpy(node_ids)] + self.projection_layer(
        ...
        if current_layer_num == 0:
            ...
            neighbor_node_conv_features = neighbor_node_conv_features.reshape(
                node_ids.shape[0], num_neighbors, self.output_dim
            )
