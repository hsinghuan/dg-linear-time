# Copied from TGB repo: https://github.com/shenyangHuang/TGB/blob/main/modules/edgebank_predictor.py
...
class EdgeBankPredictor:
    """Edgebank module for storing historical interactions and predicting future interactions."""
    ...
    def __init__(
        ...
        r"""
        initialize edgebank and specify the memory mode
        Parameters:
        ...
        assert memory_mode in [
            "unlimited",
            "fixed_time_window",
        ], "Invalid memory mode for EdgeBank!"
    ...
    def update_memory(self, src: np.ndarray, dst: np.ndarray, ts: np.ndarray):
        r"""
    ...
    def _update_unlimited_memory(self, update_src: np.ndarray, update_dst: np.ndarray):
        r"""
    ...
    def _update_time_window_memory(
        ...
        if update_ts.max() > self.cur_t:
            ...
            self.prev_t = (
                self.cur_t - self.duration
            )  # those edges before prev_t will be assigned zero prob
