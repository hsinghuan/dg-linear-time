...
from src.models.modules.attn import TransformerEncoder
...
class CAWN(nn.Module):
    ...
    def __init__(
        ...
        :param time_encoding_method: str, time encoding method
        :param avg_time_diff: float, average time difference
        :param std_time_diff: float, standard deviation of time difference
    ...
    def compute_node_temporal_embeddings(
        ...
        """Given node interaction time node_interact_times and node multi-hop graphs
        node_multi_hop_graphs, return the temporal embeddings of nodes :param node_interact_times:

        ndarray, shape (batch_size, )
        :param node_multi_hop_graphs: tuple of three ndarrays, each array with shape (batch_size,
            num_neighbors ** self.walk_length, self.walk_length + 1)
        :return:
        """
        ...
        ) = convert_format_from_tree_to_array(
            node_ids=node_ids,
            node_interact_times=node_interact_times,
...
def convert_format_from_tree_to_array(
    ...
    """Convert the multi-hop graphs from tree-like data format to aligned array-like format :param.

    node_ids: ndarray, shape (batch_size, ) :param node_interact_times: ndarray, shape (batch_size,
    ) :param node_multi_hop_graphs: tuple, each element in the tuple is a list of self.walk_length
    ndarrays, each with shape (batch_size, num_neighbors ** current_hop) :param num_neighbors: int,
    number of neighbors to sample for each node :return:
    """
    # tuple, each element in the tuple is a list of self.walk_length ndarrays, each with shape (batch_size, num_neighbors ** current_hop)
    nodes_neighbor_ids, nodes_edge_ids, nodes_neighbor_times = node_multi_hop_graphs
    ...
    for tree_format_data in [nodes_neighbor_ids, nodes_edge_ids, nodes_neighbor_times]:
        ...
        assert (
            ...
            and num_last_hop_neighbors == num_neighbors**walk_length
            and walk_length_plus_1 == walk_length + 1
        )
        # record the information of random walks with num_last_hop_neighbors paths, where each path has length walk_length_plus_1 (include the target node)
        # ndarray, shape (batch_size, num_last_hop_neighbors, walk_length_plus_1)
...
class PositionEncoder(nn.Module):
    ...
    def forward(self, nodes_neighbor_ids: np.ndarray):
        """Compute the position features of nodes in nodes_neighbor_ids :param nodes_neighbor_ids:
        ndarray, shape shape (batch_size, num_neighbors ** self.walk_length, self.walk_length + 1)
        :return:
...
class WalkEncoder(nn.Module):
    """Walk encoder that first encodes each random walk by BiLSTM and then aggregates all the walks
    by the self-attention in Transformer."""
    ...
    def forward(
        ...
        """First encode each random walk by BiLSTM and then aggregate all the walks by the self-
        attention in Transformer :param neighbor_raw_features: Tensor, shape (batch_size,
        num_neighbors ** self.walk_length, self.walk_length + 1, node_feat_dim) :param
        neighbor_time_features: Tensor, shape (batch_size, num_neighbors ** self.walk_length,
        self.walk_length + 1, time_feat_dim) :param edge_features: Tensor, shape (batch_size,
        num_neighbors ** self.walk_length, self.walk_length + 1, edge_feat_dim) :param
        neighbor_position_features: Tensor, shape (batch_size, num_neighbors ** self.walk_length,
        self.walk_length + 1, position_feat_dim) :param walks_valid_lengths: ndarray, shape
        (batch_size, num_neighbors ** self.walk_length), record the valid length of each walk
        :return:"""
...
class BiLSTMEncoder(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, lengths: np.ndarray):
        """Encode the inputs by BiLSTM encoder based on lengths :param inputs: Tensor, shape
        (batch_size, num_neighbors ** self.walk_length, self.walk_length + 1, input_dim) :param
        lengths: ndarray, shape (batch_size, num_neighbors ** self.walk_length), record the valid
        length of each walk :return:"""
