...
class GraphMixer(nn.Module):
    ...
    def compute_src_dst_node_temporal_embeddings(
        ...
        node_interact_times: ndarray, shape (batch_size, ) :param num_neighbors: int, number of
        neighbors to sample for each node :param time_gap: int, time gap for neighbors to compute
        node features :return:"""
    ...
    def compute_node_temporal_embeddings(
        ...
        """Given node ids node_ids, and the corresponding time node_interact_times, return the
        temporal embeddings of nodes in node_ids :param node_ids: ndarray, shape (batch_size, ),
        node ids :param node_interact_times: ndarray, shape (batch_size, ), node interaction times
        :param num_neighbors: int, number of neighbors to sample for each node :param time_gap:

        int, time gap for neighbors to compute node features
        :return:
        """
        # link encoder
        ...
        nodes_neighbor_time_features = self.time_encoder(
            timestamps=torch.from_numpy(node_interact_times[:, np.newaxis] - neighbor_times)
            .float()
            .to(self.device)
        )
        ...
        )  # time_gap is T in the original paper
...
class FeedForwardNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        """Feed forward net forward process :param x: Tensor, shape (*, input_dim) :return:"""
        return self.ffn(x)
...
class MLPMixer(nn.Module):
    ...
    def forward(self, input_tensor: torch.Tensor):
        """Mlp mixer to compute over tokens and channels :param input_tensor: Tensor, shape
        (batch_size, num_tokens, num_channels) :return:"""
        # mix tokens
