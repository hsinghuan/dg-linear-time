...
from src.models.modules.cawn import CAWN, convert_format_from_tree_to_array
from src.models.modules.mlp import MergeLayer


class CAWNModule(LinkPredictor):
    """LightningModuel for CAWN."""
    ...
    def __init__(
        ...
        sample_neighbor_strategy: str = "time_interval_aware",
    ...
    def setup(self, stage: str):
        ...
        if self.hparams.scale_timediff:
            print("Start calculating avg and std of time differences")
            ...
            node_multi_hop_graphs = self.train_neighbor_sampler.get_multi_hop_neighbors(
                num_hops=self.hparams.walk_length,
                node_ids=node_ids,
                node_interact_times=node_interact_times,
                num_neighbors=self.hparams.num_neighbors,
            )
            _, _, nodes_neighbor_times = convert_format_from_tree_to_array(
                node_ids=node_ids,
                node_interact_times=node_interact_times,
            ...
            assert (
                ...
                == node_interact_times.repeat(
                    repeats=self.hparams.num_neighbors**self.hparams.walk_length, axis=0
                ).reshape(
                    len(node_interact_times),
                    self.hparams.num_neighbors**self.hparams.walk_length,
                )
            ).all()
            ...
            avg_time_diffs = np.nanmean(nodes_neighbor_delta_times, axis=1)
            self.avg_time_diff = np.nanmean(avg_time_diffs)
            self.std_time_diff = np.nanstd(avg_time_diffs)
        ...
        backbone = CAWN(
            ...
            position_feat_dim=self.hparams.position_feat_dim,
            walk_length=self.hparams.walk_length,
            num_walk_heads=self.hparams.num_walk_heads,
    ...
    def _pred_scores(
        ...
        **kwargs,
        ...
        """Predict the scores of src[i] and dst[i] happening at time t[i]."""
